...
export class PaymentOptionPage implements OnInit {
  ...
  async makePaymentWithStripe() {
    ...
    (async () => { this.http.post<{
      ...
      // subscribe(data => {
      //   // this.paymentIntent = data.data.client_secret
      //   // this.CallForStripePayment(data.data.client_secret);\
      //   console.log(data)
      // })
      // }>("http://barter-tech.antino.ca/api/testingIntent", {}).pipe(first()).toPromise(Promise);
    })();
  }
