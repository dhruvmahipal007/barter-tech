...
export class PreorderPage implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.preOrderobject) {
      this.isFormVisible = true;
      this.confirmationForm.controls['selectedDate'].patchValue(
        this.preOrderobject.selectedDate
      );
      this.confirmationForm.controls['selectedTime'].patchValue(
        this.preOrderobject.selectedTime
      );
      this.todaydate = this.preOrderobject.selectedDate;
      this.confirmationForm.controls['selectedPeople'].patchValue(
        this.preOrderobject.selectedPeople
      );
  ...
  async onSubmit() {
    ...
    if (this.currentRoute != this.value && this.cartItems.length > 0) {
      const alert = await this.alertController.create({
        header: 'Alert',

        message:
          'Are you sure you want to proceed as items present in the cart will be removed',
        buttons: [
          {
            text: 'Cancel',
            role: 'cancel',
  ...
  async checkPastTiming(event) {
    ...
    let selectedDate = new Date(
      this.confirmationForm.controls['selectedDate'].value
    );
    ...
    var d2 = new Date(
      ...
      parseInt('01', 10) - 1,
      parseInt('01', 10),
      parseInt(currentTime[0], 10),
      parseInt(currentTime[1], 10)
    );
    var dd1 = d1.valueOf();
    var dd2 = d2.valueOf();

    if (dd1 < dd2 && todayDate > selectedDate) {
      const alert = await this.alertController.create({
        header: 'Alert',
  ...
  async onTimeChange(event) {
    ...
    if (
      this.confirmationForm.controls['selectedTime'].value != null &&
      this.confirmationForm.controls['selectedTime'].value != undefined
    ) {
      let selectedDate = new Date(
        this.confirmationForm.controls['selectedDate'].value
      );
      ...
      if (
        Number(openingHrs[0]) > this.hours ||
        (this.hours >= Number(closingHrs[0]) && minutes >= closingHrs[1])
      ) {
        const alert = await this.alertController.create({
          ...
          message:
            this.currentRoute +
            ' service is not available during this time. Please choose another time',
          buttons: [
            {
              text: 'Cancel',
              role: 'cancel',
  ...
  checkValidity(data, day) {
    ...
    keys.map((o, i) => {
      if (this.currentRoute == o.toLowerCase()) {
        ...
        this.currentRoute == 'dinein' &&
        o.toLowerCase() == 'business'
      ) {
        timing = this.commonMethods(data[o], day);
      }
    });
